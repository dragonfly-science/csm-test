...
Footnote to included in table header: \footnote{Available here: \url{https://www.arcgis.com/home/item.html?id=fd61b9e0c69c4e14bebd50a9a968348c}}

\begin{tiny}
<<r1, echo=FALSE>>=
sentinelbands <- read.csv(file="../tables/sentinel2.csv",
                                   header = TRUE)
knitr::kable(sentinelbands, col.names = c('Band', 'Description', 'Wavelength (µm)','Resolution (m)'),caption='Overview Sentinel-2 bands',longtable=T, row.names=F)
@
\end{tiny}
...
vegindices <- read.csv(file="../tables/vegindices.csv",
                                   header = TRUE)
knitr::kable(vegindices, col.names = c('Vegetation Indices', 'Equations', 'References'),caption='Vegetation Indices derived from Sentinel 2 information (adapted from Pandit
et al. 2018, Wang et al. 2020)',longtable=T, row.names=F)
@
\end{tiny}
...
B) \textbf{LiDAR and Hyperspectral} combination has lower accuracy than LiDAR alone in tropical forest Costa Rica \citep{clark_estimation_2011}.
...
lidarmetrics <- read.csv(file="../tables/lidarmetrics.csv",
                                  header = TRUE)
knitr::kable(lidarmetrics, col.names = c('LiDAR', 'Variable', 'Definition'),caption='Overview LiDAR derived metrics (taken from Wang et al. 2020)',longtable=T, row.names=F)
@
\end{tiny}
...
\paragraph{Problems with field data} (taken from \cite{lu_survey_2016}: \
...
landsat <- read.csv(file="../tables/landsat.csv",
                                  header = TRUE)
knitr::kable(landsat, col.names = c('Band', 'Wavelength (µm)','Resolution (m)'),caption='OLandsat 8-9 Operational Land Imager (OLI) and Thermal Infrared Sensor (TIRS)',longtable=T, row.names=F)
@
\end{tiny}
\label{fig:landsat}

reference to include in caption: \cite{usgs_what_2018}
...
\paragraph{Normalized Burn Ratio (NBR) for LandSat time series analysis} \
...
\begin{figure}[H]
  \includegraphics[scale=0.5]{../figures/nbrtimeseries.png}
