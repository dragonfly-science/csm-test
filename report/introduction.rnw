...
fine-scale land-cover model capable of detecting ``tree-approximate objects'', the
Canopy Height Model. The model is built on 
 high-quality
LiDAR (light detection and ranging) data.

The model's poential to support data-informed decision-making 
has prompted interest in 
expanding its scope to the entirety of the State of California, United States.
This expansion requires high-quality LiDAR data over the state's spatial extent; however,
a uniform LiDAR dataset does not currently exist for the entirety of the state.

The current project explored approaches for generating the required land-cover dataset, using publicly-available data. The overall aim was to provide the closest approximation of relevant information that would generally be obtained by the collection of LiDAR data.

 This report provides an overview of this exploration and proof-of-concept.
