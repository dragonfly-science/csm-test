...
a uniform LiDAR dataset does not currently exist for the entirety of the state. A small number of  CHM's for the State of California are freely available, but provided layers are lacking transparency about their creation methods While Vibrant Planet has existing worfklows to derive CHM's, the application is limited by the availability of vendor provided Digital Terrain Models (DTM). Currently a significant number of raw classified point clouds are publicly accessible (e.g. from United States Geological Survey (USGS)) and hence provide an opportunity to develop new processes to classify points and create a scalable, replicable, and repeatable approach. Within this report we will present and evaluate our own workflows to generate CHM to operate independently from current limitations.

Once workflows to generate CHM's from raw point clouds are established, the information can be upscaled by training data layers which extend over the entire state. Landsat imagery is the method of choice to generate landscape level datasets for forest monitoring \citep{matasci_large-area_2018,pham_monitoring_2017, huang_integration_2019, ghosh_aboveground_2018} and novel workflows to develop time series analysis have been developed \citep{white_nationwide_2017, wulder_satellite-based_2020, hermosilla_mass_2016}. Currently, the methodologies are significantly limited by the spatial resolution of the Landsat pixels (30 m) and few papers have discussed training of satellite information with LiDAR \citep{latifi_stratified_2015, wangda_species_2019, wang_estimating_2020}. In 2017 the European Space Agency opened the Sentinel-2 image library to the public, offering new opportunities to refine monitoring approaches at higher resolutions (10 - 20 m) and band selection. However, implementation of the Sentinel-2 datasets pose a significant challenge in terms of data ingestion, reprojection and cleaning which have yet to be explored. 

% add forest fire context?

This report provides an overview of the approach and datasets used to develop the CHM test case and the consecutive upscaling of LiDAR derived canopy height information via Sentinel-2 imagery. A detailed documentation for geospatial processes, machine-learning concepts and data ingestions via Amazon Web Services (AWS) that were part of this project are available in the Appendix and linked repositories.
