...
Canopy Height Model (CHM), which is capable of detecting ``tree-approximate objects''. The model is built on 
 high-quality LiDAR (light detection and ranging) data.
...
The current project was focused on a proof-of-concept for generating a land-cover dataset and expanding the CHM, using publicly-available data. 
The overall aim was to provide the closest approximation of relevant information that would generally be obtained by the collection of LiDAR data.  

Although there is a small number of freely available CHM models for the State of California, the available data are lacking transparency about their creation methods or are only available in small downloads. Because United States Geological Survey (USGS) makes a significant number of raw classified point clouds publicly accessible, the current project was based on developing workflows to generate a CHM. This developed CHM assures a scalable, replicable, and repeatable approach to the methods used, with a test case providing a proof-of-concept.  The test case used all currently available LiDAR point clouds for a given region from the Opentopography database\footnote{Opentopography LiDAR database: \url{https://portal.opentopography.org/datasets}}. These point clouds date back to 2010 and vary in resolution, sampling technique, and provider. 

This report provides an overview of the datasets used to develop the CHM test case, and presenting a proof-of-concept.  Detailed documentation for geospatial processes, machine-learning concepts and data ingestions via Amazon Web Services (AWS) are available on the linked repositories.
