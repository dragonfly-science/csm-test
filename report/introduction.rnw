...
Canopy Height Model (CHM), which is capable of detecting ``tree-approximate objects''. The model is built on 
 high-quality LiDAR (light detection and ranging) data. LiDAR measurements (point clouds) reflect state of the art technology for calculating above-ground biomass \citep{chan_estimating_2021}, 
...
which fail to accurately depict vegetation surface information \citep{lu_survey_2016}.  
...
a uniform LiDAR dataset does not currently exist for the entirety of the state. At the same time, 
data from other sources may be available to generate a land-cover dataset and for expanding the CHM.

A small number of  CHM models for the State of California are freely available, but the available data are lacking transparency about their creation methods, or they are only available in small downloads. At the same time, a significant number of raw classified point clouds are publicly accessible (from United States Geological Survey (USGS)).  These publicly-available data provide an opportunity of developing a CHM that assures  a scalable, replicable, and repeatable approach. 

The current project was based on developing an approach to generate this type of CHM, including a test case to provide a proof of concept.  The test case used all currently available LiDAR point clouds for a given region from the Opentopography database\footnote{Opentopography LiDAR database: \url{https://portal.opentopography.org/datasets}}. These point clouds date back to 2010, and vary in resolution, sampling technique, and provider.  
...
This report provides an overview of the approach and datasets used to develop the CHM test case.   Detailed documentation for geospatial processes, machine-learning concepts and data ingestions via Amazon Web Services (AWS) that were part of this project are available on the linked repositories.

%add a bit more in once the overall report become clear
%context for forest fires?
