...
A small number of  CHM models for the State of California are freely available, but the available data are lacking transparency about their creation methods, or they are only available in small downloads. At the same time, a significant number of raw classified point clouds are publicly accessible (from United States Geological Survey (USGS)).  These publicly-available data provide an opportunity for developing a CHM that assures  a scalable, replicable, and repeatable approach. 

The current project was based on developing an approach to generate this type of CHM, including a test case as a proof of concept.  %needs re-wording
