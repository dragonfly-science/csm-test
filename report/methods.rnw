...
Our goal is simply to compare pixel to pixel values, noting the difference between those values, determining if those values fall into a comfortable range of difference. Output models are first compared visually to see if we are moving the right direction with our processes.  We check for visual simlarities between the datasets, e.g. basic tree shapes, structures, and objects align.  Next, we move to individual pixel comparisons by hand checking low and high points for randomely sected areas.  Finally, we move to raster calculations differencing the entire datasets looking for mean difference in height and checking the standard deviations. 
