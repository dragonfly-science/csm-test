...
In our project, we are implementing PDAL for point cloud filtering plus, generation of Digital Terrain Models (DTM) and Digital Surface Models
...
\begin{enumerate}
    ...
    \item Generate Digital Terrain Model (DTM) from ground points
    \item Generate Digital Surface Model (DSM) from highest points
