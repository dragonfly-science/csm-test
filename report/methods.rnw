...
Since we are generating surfaces from raw LAS/LAZ point clouds, we needed a method to compare our outputs to existing models.  
...
Our goal is simply to compare pixel to pixel values, noting the difference between those values, determining if those values fall into a comfortable rangle of difference. It is important to note that no CHM model is perfect and no output can definitively claim to be a perfect model.
